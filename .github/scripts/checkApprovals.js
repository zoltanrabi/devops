...
async function run() {
  try {
    ...
    const validateWorkflow = runs.workflow_runs.find(run => run.name === 'Validate Test Pull Request' && run.head_sha === pr.head.sha);

    if (!validateWorkflow || validateWorkflow.conclusion !== 'success') {
      core.setFailed('The Validate Test Pull Request workflow has not passed.');
      return;
    }
